...
class TestParseData():
	...
	def test_find_emails(self):
		assert(self.pd.find_emails('tests/resumes/julie.doc') == "jha12@zips.uakron.edu")

		assert(self.pd.find_emails('tests/resumes/kyle.docx') == "Kjv13@zips.uakron.edu")
		assert(self.pd.find_emails('tests/resumes/russell.pdf') == "Rwr21@zips.uakron.edu")
